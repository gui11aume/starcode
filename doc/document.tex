...
sequences because in this case $k$ is large. The probability of an
exact match between subsequences of length $k$ is small and the
trie-search will be avoided for many such queries. The absence of match
is detected at the computational cost of several table lookups, compared
to the higher cost of measuring the Levenshtein distance between long
sequences.
...
in 4 minutes and 32 seconds without a single error (the output
...
are summarized in Figure~\ref{fig:04}. Both algorithms rely on
...
sequences. Figure~\ref{fig:04}c shows the running time of starcode along with the
...
this test) to index and run the query proper. Figure~\ref{fig:04}d shows the
lower bound on
...
\textit{Neisseria meningitidis} (ATGCCGTCTGAA) as the most abundant 12-mer, comprising
...
To test starcode on the scale of a metazoan genome, we used a \textit{Drosophila}
...
The total coverage of Red and Yellow chromatin in the \textit{Drosophila} genome is
...
Red and 70 minutes for Yellow chromatin using 20 threads). The most discriminating
18-mer was (GA)$_9$, which showed a 4.8 fold enrichment in Red \textit{versus} Yellow
chromatin.
...
regions found in two different chromatin types in \textit{Drosophila}. GA
repeats are known to be bound \textit{in vivo} by the protein GAF
\citep{pmid12601174}, which suggests
