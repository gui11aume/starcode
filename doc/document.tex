...
If the Levenshtein distance between two sequences of length $L$ is
less then or equal to $\tau$, then there must exist a perfect
match of length at least $k = L / (\tau+1)$, assuming for the time being
that $k$ is an integer.
We take advantage of this property to build a lookup table containing
all such $k$-mers from sequences inserted in the trie. When
a new sequence is queried, subsequences are first extracted from it
and queried against the lookup table. If none of the subsequences
are found, the minimum distance between the query and all the
sequences in the trie is at least $\tau+1$, and the trie search can
therefore be omitted. This lookup is particulary useful for long query
sequences (see \ref{scalability}). In this case $k$ is large, so the
probability of an exact match between subsequences of length $k$ is small.
The absence of match is detected at the computational cost of several
table lookups, compared to the higher cost of measuring the Levenshtein
distance between long sequences.

In general $L / (\tau+1)$ is not an integer. To deal
with this complication, we divide the query into $\tau+2$ subsequences.
The first $k+1$ are called words and have target length
$(L-\tau) / (\tau+1)$. The last subsequence is called the tail and has
length $\tau$ for reasons explained below. The length of the words is
computed by integer division, and the remainder $R$ is distributed
equally among the last $R$ words. As a result, the words have a length
that differs by at most 1, with a total length equal to $L-\tau$.

A word at the beginning of the query is not a match for the same
word at the end of a sequence in the trie. For this reason, we query
the words against $k+1$ distinct lookup tables. In order to allow
insertions and deletions, the lookup search must also query shifted
versions of the words. In the worst case, only one word is found and the
mismatches are evenly distributed in the query sequence. The maximum
number of cumulative insertions/deletions at the $i$-th word
is $i-1$. Hence, the $i$-th word has to be shifted and queried against
the lookup table $2(i-1)+1$ times to cover all the possible cases.
The last word will therefore be queried $2\tau+1$ times, including
$\tau$ shifts on the right and $\tau$ shifts on the left. For the latter
we use the $\tau$ nucleotides stored in the tail.
...
an independent trie is created and filled with the sequences
of one block using the seek and construct process described above. In the second
phase, blocks of sequences are queried against but not inserted into the tries
built in the first phase. If the
...
is assigned to a cluster if all its read counts are eventually given to the
...
\cite{pmid23953119} which is implemented in R and relied on the \texttt{stringdist}
...
\label{scalability}
Write some crap here.

\begin{figure}[!tpb]%figurex
\centerline{\includegraphics[scale=0.47]{scalability.pdf}}
\caption{Scalability. Top left: the running time of starcode is
approximlately $O(n^{1.7})$, where $n$ is the number of sequences to
cluster. Top right: the running time is very sensitive to the maximum
Levenshtein distance used in custering.  Bottom left: the running time
as a function of the length of the sequences has a complex behavior. It
increases initially but then decreases for long sequences because the
lookup search becomes more effective. Bottom right: the scaling of the
relative performance is sub-linear as the number of threads increases.
The algorithm is fully parallel, but the memory bandwidth becomes
limiting on the tested hardware.
}\label{fig:X}
\end{figure}
...
We generated 1 million random 40-mers that were duplicated 47 times.
For each 40-mer, we inserted 3 addional mutant sequences to the pool
...
distance 3 from the canonical representant.

Using 12 cores, starcode clustered the shuffled 50 million sequences
in 5 minutes and 17 seconds without a single error (the output
consisted of 1 million clusters of size 50).  The running times
...
algorithms in speed and accuracy to cluster random sequences.
...
UCLUST on the dataset described in the text. \textbf{b} Error rate
compared to CD-HIT and UCLUST, expressed as the percentage of clusters
...
than 45 seconds with 12 cores. We identified the known DNA uptake sequence of
\textit{Neisseria meningitidis} (ATGCCGTCTGAA) as the most abundant 12-mer.
This result testifies to the fact that starcode can be used to identify biologically
...
For each replicate, we replaced the RNAcompete signals by their rank and
...
were found using the sphere clustering of starcode with max Levenshtein
distance 2. The score of the most enriched 10-mer is thus the sum of the
ranks of all 10-mers within this distance. Clustering the 6.3 million
extracted 10-mers with 12 cores took about 20 seconds for each replicate.
The most enriched 10-mers were AGGACACGGA, AGGACACGGA, AGGACGGAGG,
AGGACGGAGG, AGGACACGGA and AGGATACAGG. Except for the last replicate, the
motifs consist of AGGAC and GGA, with a spacer of variable length. This
suggests that the binding of SRSF1 to RNA is flexible, which explains the
poor agreement between the motifs derived from 6-mers or 7-mers.
