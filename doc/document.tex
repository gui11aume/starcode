...
and deletions \citep{pmid19023044}. The enormous throughput of such technologies
...
from the same canonical sequence. 
...
\begin{tabular}{l|l}
    ...
    cd-hit-est & \texttt{cd-hit-est -n 9 -c 0.9 -M 0 -r 0}\\
...
\begin{tabular}{l|l}
    ...
    cd-hit-est & \texttt{cd-hit-est -n 8 -c 0.94 -M 0}\\
    seed & \texttt{SEED --mismatch 3 --shift 3}\\ 
    rainbow & \texttt{rainbow cluster -m 3}
\end{tabular}
%}
\label{tab:params2}
\end{table}
...
FASTQ (in which case starcode ignores the quality). Starcode has two
output formats: cluster and non-redundant. In cluster mode, starcode
...
sequences. In non-redundant mode, only the canonical sequences are
printed. The second mode is especially useful to filter out redundant
...
(sequencing errors, PCR mutations \textit{etc}.), a more general
...
test, only one parameter is modified while the others are kept constant.
...
reads, for this reason all the tools could not be run no the same
dataset.
...
the generated data is single reads. In addition, evaluating the
exactness of slidesort 
...
time of slidesort and starcode show a linear and sub-linear trends,
...
full graph necessary for clustering (which is by far the most memory
demanding structure). The memory usage of starcode is the highest for
clusters of size 1000, but it decreases and becomes lower than the
memory usage of seed as the size of the cluster increases.
...
nucleotides were included in the file. Dataset 2 (SRRHENCHI) consists
...
\begin{tabular}{l|l c r}
    ...
    SRR950457 & 6,542,309    & 16     & single     \\
    SRRxxxxxx & 127,675,537  & 50     & single     \\
    SRR950477 & 2,460,226    & 50+50  & paired-end \\
\end{tabular}
...
the option '-d 2' and the identity for cd-hit was set to '-c 0.85'. We
were not able to run seed on dataset 1 due to limitations on the
minimum input sequence length. Starcode was faster than the other
tools on all the datasets. CD-HIT and seed came in second position
...
\begin{tabular}{l|c c c}
    \textit{software}   & \textit{SRR950457} &
      \textit{SRRxxxxxx} & \textit{SRR950477} \\ \hline
    starcode   & 5           & 2,898       & 44    \\
    seed       & -           & 60,374      & -     \\
    sidesort   & 4,055       & $>$ 10 days & -     \\
    rainbow    & -           & -           & 306   \\
...
cd-hit used the same amount of memory on dataset 2. Both needed twice
as much memory as slidesort, which runs in advantage because it does
not need to keep the complete graph during the all-pair comparison
step to perform future clustering.
...
Starocde is a solid algorithm for sequence clustering based on
