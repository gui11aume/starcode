...
(see \ref{scalability}), where $k$ is large and the probability
...
\label{scalability}
In Figure~\ref{fig:X} we present an evaluation of the performance and
scalability of starcode as a function of many critical parameters. The
scalability tests were performed using an **artificial set of random
sequences**. The sequence length and the number of input sequences
varies depending on the test. The standard configuration consists of a
set of **X** sequences of length **Y** running on **Z** threads and a
cluster distance of **D**. To test the scalability of starcode as a
function of a single parameter, only the selected value is modified
whereas the others are kept constant. 

In Figure~\ref{fig:X}a we show the running time as a function of the
selected Levenshtein distance for the clusters. The evolution with the
increasing distance shows the common exponential trend. Note, however,
that for distances below 3 the running time remains constant. This is
due to the fact that the average sequence distance of the input
sequences lies above this threshold. Consequently, the constant part
is clearly dependent on the diversity of the input set and when the
average distance between sequences is reached, the running time scales
exponentially.

In Figure~\ref{fig:X}b we measure the running time of starcode as a
function of the number of input sequences $n$. Both axes are in
logarithmic scale, drawing an evident linear evolution. Based on this
result, we conclude that the running time of starcode is approximately
$O(n^{1.7})$.

To prove the effectiveness of the parallel processing, we ran starcode
over the same dataset using different number of threads. As shown in
Figure~\ref{fig:X}c, the algorithm is fully parallel as the relative
performance increases linearly up to 4 threads. The curve bending
produced thereafter is an artifact of having insufficient memory
bandwidth to satisfy the increased demand of memory accesses and it is
due to hardware limitations.

Finally, in Figure~\ref{fig:X}d we investigate the execution time
with varying sequence lengths. It increases initially due to the
ineffectiveness of the lookup table and the limited diversity of short
sequences, i.e. short sequences are less diverse, yielding a bigger
clustering rate. At a certain length, the lookup search algorithm
starts to be effective and boosts the performance since most of the
queries are resolved without the need for trie-search.

\begin{figure}[!tpb]%figurex
\centerline{\includegraphics[scale=0.47]{scalability.pdf}}
\caption{Scalability. a. Running time as a function of the clustering
  distance. b. Logarithm of the running time versus the logarithm of
  the number of sequences to be clustered. c. Relative performance increase for
  different number of parallel threads. d. Running time versus length
...
E5-2620 system with **64~GB** of DDR3-RAM at 1333~Mhz. The execution
options for each software are summarized in
Table~\ref{tab:soft-options}. Denoted as \emph{sequential R} is the
